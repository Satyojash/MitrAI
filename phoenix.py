...
if system_os == 'Linux':
    ...
    raise NotImplementedError(f"Unsupported OS: {system_os}")

def speak(audio):
    engine.say(audio)
    engine.runAndWait()

satyamhehe = ["yes, what can i help you with", "at your service, operator", "ummmmmm..., yes?", "Yes, I am listening.", "HI, how can i help u today"]

ojashhehe = random.choice(satyamhehe)

def call(wake_words=["mitra", "buddy", "wake up", "phoenix"]):
    r = sr.Recognizer()
    pattern = re.compile("|".join(re.escape(word) for word in wake_words), re.IGNORECASE)
    while True:
...
def searchYoutube(query):
    query = query.lower().replace("youtube search", "").replace(
        "search youtube", "").replace("youtube", "").replace("for","").strip()
